...
ResidualAbsTol = 1e-3;
SolRelTol = 1e-3;
IterativeSolverIterations = 30;
IterativeSolverRelTol = 1e-2;
PerturbationRelTol = 1e-7;
MaxStep = [1e10;1e10];
NumSteps = 15;
for k=1:2:length(varargin)
    ...
    elseif strcmp(a,'SolRelTol')
        SolRelTol = b{1};
    elseif strcmp(a,'IterativeSolverIterations')
        IterativeSolverIterations = b{1};
    elseif strcmp(a,'IterativeSolverRelTol')
        IterativeSolverRelTol = b{1};
    elseif strcmp(a,'PerturbationRelTol')
        PerturbationRelTol = b{1};        
...
if isempty(uoPDE)
    uoGeometry = coeff.geometry.mesh.p(1:2,:)';
...
J = [];
    function rhs = computeRHS(u)
        ...
        p = uu(:,1:2)';
        if UpdateGeometry
            [K,M,F] = coeff.getMatrices('solution',uPDE-uoPDE(1:N*numNodes),'p',p);
            [Q,G,H,R] = bc.getMatrices('solution',uPDE-uoPDE(1:N*numNodes),'p',p);
        else
            [K,M,F] = coeff.getMatrices('solution',uPDE-uoPDE(1:N*numNodes));
            [Q,G,H,R] = bc.getMatrices('solution',uPDE-uoPDE(1:N*numNodes));
        end
        if isempty(find(H ~= 0))
            H = [];
            R = [];
        ...
        [jK,jM] = coeff.getJacobian('solution',uPDE-uoPDE(1:N*numNodes));
        J = [jK+jM+Q+Qadd H.';H sparse(sizeH,sizeH)];
    end
    function Jx = computeJx(x)
        if norm(x) == 0
            Jx = 0*Jxb;
        else
            ratio = PerturbationRelTol*norm(xb)/norm(x);
            Jx = (computeRHS(xb + ratio*x) - Jxb)/ratio;
        end
    end
    function InvJbx = computeInvJbx(x)
        InvJbx = QJb*(UJb\(LJb\(PJb*(RJb\x))));
    end
uo = uoPDE;
u = uinitialPDE;
converged = false;
...
while ~converged && iter < NumSteps
    ...
    xb = u;
    du = J\(-b);
%     [LJb,UJb,PJb,QJb,RJb] = lu(J);
%     du = gmres(@computeJx,-b,IterativeSolverIterations,IterativeSolverRelTol,1,@computeInvJbx);
