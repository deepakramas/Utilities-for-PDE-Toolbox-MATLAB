% modified to handle geometries created by geometryObject
%  
...
%       $Revision: 1.8.4.3 $  $Date: 2010/10/08 17:14:48 $

if nargin<1 || nargin>3,
  error(message('pde:pdeigeom:nargin'));
end

if ischar(dl) || isa(dl,'function_handle'),
  if nargin==1,
    x=feval(dl);
  elseif nargin==2,
    x=feval(dl,bs);
...
if find(bs1<1 | bs1>nbs),
  error(message('pde:pdeigeom:InvalidBs'))
end
...
elseif m~=size(s,1) || n~=size(s,2),
  error(message('pde:pdeigeom:SizeBs'));
end
  ...
  for k=1:nbs,
    ...
    if ~isempty(ii),
      ...
      elseif dl(1,k)==4                 % Ellipse fragment
        ...
        error(message('pde:pdeigeom:InvalidSegType'));
      end
    end
  end
end
